...
import { ChevronRight, Clock, Users, BarChart3, Play, Lock, CheckCircle, Circle, Instagram, CheckCircle2 } from 'lucide-react';
...
const CoursePage: React.FC = () => {
  ...
  const getModuleLessons = (moduleId: string) => courseLessons.filter(l => l.moduleId === moduleId).sort((a, b) => a.order - b.order);
  const isLessonCompleted = (lessonId: string) => userProgress?.completedLessons.includes(lessonId) || false;
  ...
  const learningPoints = ['Fundamentos e conceitos essenciais', 'Estratégias práticas de implementação', 'Técnicas avançadas de otimização', 'Casos de estudo reais', 'Templates e recursos exclusivos', 'Suporte da comunidade'];
        ...
        <div className="container py-12">
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-1 space-y-6">
              ...
              {/* Progress */}
              {completedLessons > 0 && <div className="bg-card/50 rounded-xl p-4 border border-border">
              ...
              {/* CTA */}
              <Button size="lg" onClick={handleStartCourse} className="text-lg px-8 py-6">
            ...
            <div className="lg:w-96 rounded-xl overflow-hidden shadow-elegant border border-border">
              <img src={course.thumbnail || 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&h=600&fit=crop'} alt={course.title} className="w-full aspect-video object-cover" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 space-y-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {learningPoints.map((point, idx) => <div key={idx} className="flex items-start space-x-3 p-4 rounded-lg bg-card border border-border">
            ...
            <section>
              ...
              <Accordion type="multiple" className="space-y-4">
                ...
                return <AccordionItem key={module.id} value={module.id} className="bg-card border border-border rounded-xl overflow-hidden">
                      <AccordionTrigger className="px-6 py-4 hover:bg-accent/30 transition-colors [&[data-state=open]]:bg-accent/30">
                        <div className="flex items-center justify-between w-full pr-4">
                          <div className="flex items-center space-x-4">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${moduleCompleted ? 'bg-success/20' : 'bg-accent'}`}>
                              {moduleCompleted ? <CheckCircle className="w-4 h-4 text-success" /> : <span className="text-sm font-medium text-foreground">{module.order}</span>}
                            </div>
                      ...
                      <AccordionContent className="px-0 pb-0">
                        ...
                        return <Link key={lesson.id} to={`/course/${courseId}/lesson/${lesson.id}`} className="flex items-center space-x-4 px-6 py-4 border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                                <div className="flex-shrink-0">
                                  {isComplete ? <CheckCircle className="w-5 h-5 text-success" /> : lesson.locked ? <Lock className="w-5 h-5 text-destructive" /> : <Circle className="w-5 h-5 text-muted-foreground" />}
                                </div>
