...
const AdminChallengesManager: React.FC = () => {
  ...
  const openModal = (challenge?: HofChallenge) => {
    if (challenge) {
  ...
  const updateVideo = (index: number, field: keyof ChallengeVideo, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      videos: prev.videos.map((v, i) => 
  ...
  const calculateTotalDuration = (videos: ChallengeVideo[]): number => {
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="bg-popover border-border">
                    <DropdownMenuItem onClick={() => openModal(challenge)} className="cursor-pointer">
                      <Edit className="w-4 h-4 mr-2" />
                      Editar
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => duplicateChallenge(challenge)} className="cursor-pointer">
...
export default AdminChallengesManager;
